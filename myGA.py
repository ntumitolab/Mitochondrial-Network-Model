...
class param:
    ...
    def update_score(self):
        ...
        self.ans = [avgdeg, Ng]
        self.score = math.sqrt((self.ans[0]-self.true_ans[0])**2 + (self.ans[1]-self.true_ans[1])**2)
        # print('Ending Update')

class GA:
    def __init__(self, iter_setting, true_ans, limit1 = 1048575, limit2 = 1048575, level = 100, populations = 50, crossover_times = 20, crossover_length = 5, mating_pool_num = 40, mutate = 0.02):
        self.iter_setting = iter_setting # a list, [Initialized total nodes, # of Iteration]
        self.true_ans = true_ans
    ...
    def _init_Params(self):
        ...
        while x < self.populations:
            ...
            Params.append(param([candidate_c1 / 1000000, candidate_c2 / 1000000], self.iter_setting, self.true_ans))
            x += 1
        return Params 
    ...
    def change_to_code(self, param):
        ...
        code = list(str(self.decimal_to_binary(large_param[0])).zfill(20) + str(self.decimal_to_binary(large_param[1])).zfill(20))
        return code
    ...
    def change_to_param(self, code):
        ...
        param = [self.binary_to_decimal(large_c1) / 1000000, self.binary_to_decimal(large_c2) / 1000000]
        return param
    ...
    def _sort(self, Params):
        ...
        sorted_Scores_index = sorted(range(len(Scores)), key = lambda k : Scores[k], reverse = False)
        sorted_Params = []
        sorted_Scores = []
        for i in range(len(Params)):
    ...
    def _delete(self, Params):
        new_Params = []
        for i in range(len(Params)):
            if (Params[i].p[0] <= (self.limit1/1000000)) & (Params[i].p[1] <= (self.limit2/1000000)):
                new_Params.append(Params[i])
        return new_Params
 
    def roulette(self, Params):
        cumulation = [] 
        cumulation.append(0) # 先補一個0，等等寫 if  r >= cumulation[j] and r <= cumulation[j+1]: 那裡比較方便
        mating_pool = []
    ...
    def _crossover_and_mutate(self, mating_pool):
        ...
        for _ in range(self.crossover_times):
            ...
            child1[index_start:index_start + self.crossover_length] = mother_list[index_start:index_start + self.crossover_length]
            child1[index_start + self.crossover_length:] = father_list[index_start + self.crossover_length:]
            child2[:index_start] = mother_list[0:index_start]
            child2[index_start:index_start + self.crossover_length] = father_list[index_start:index_start + self.crossover_length]
            child2[index_start + self.crossover_length:] = mother_list[index_start + self.crossover_length:]
            ...
            child1_param = param(self.change_to_param(child1), self.iter_setting, self.true_ans)
            child2_param = param(self.change_to_param(child2), self.iter_setting, self.true_ans)
    ...
    def evolution(self):
        ...
        record_dic = {'Level':[], 'c1': [], 'c2': [],'AvgDegree':[], '#Nodes_G':[],  'Score':[]}
