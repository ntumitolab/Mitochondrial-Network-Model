...
# Functions
def get_propensity(all_nodes, rate):
    x1 = len(all_nodes[0])
    x2 = len(all_nodes[1])
    x3 = len(all_nodes[2])
    
    a1, b1, a2, b2 = rate
    propensity = [a1*x1*(x1-1),
                 b1*x2,
                 a2*x1*x2,
                 b2*x3]
    return propensity
    ...
    for i in range(3):
        if i == 0:
            ...
            for k in range(len(all_nodes[0])):
                if sorted([all_nodes[0][k].name, all_nodes[0][k].nei[0].name]) not in edge_list:
                    edge_list.append([all_nodes[0][k].name, all_nodes[0][k].nei[0].name])
        elif i == 1:
            n2.extend([j.name for j in all_nodes[i]])
            for k in range(len(all_nodes[1])):
                if sorted([all_nodes[1][k].name, all_nodes[1][k].nei[0].name]) not in edge_list:
                    edge_list.append([all_nodes[1][k].name, all_nodes[1][k].nei[0].name])
        elif i == 2:
            n3.extend([j.name for j in all_nodes[i]])
            for k in range(len(all_nodes[2])):
                if sorted([all_nodes[2][k].name, all_nodes[2][k].nei[0].name]) not in edge_list:
                    edge_list.append([all_nodes[2][k].name, all_nodes[2][k].nei[0].name])
...
def get_largest_CC(all_nodes, draw = 'T'):
    ...
    if draw == 'T':
        ...
        nx.draw(G0, node_color=values, with_labels=True, font_color='white', font_size=10)
        plt.show()
...
def Iterate(c, n, iterate, plot_t0='F', plot_tend='F'): # (rate constant, # of iteration, initialized total nodes)
    ...
    # dataframe
    dic = {'Time':[], '#Nodes':[], 'Degree1':[], 'Degree2':[], 'Degree3':[],'AvgDegree':[], '#Edges':[], 
           '#Nodes_G0':[], 'Degree1_G0':[], 'Degree2_G0':[], 'Degree3_G0':[],'AvgDegree_G0':[], '#Edges_G0':[],
           '#Nodes_G1':[], 'Degree1_G1':[], 'Degree2_G1':[], 'Degree3_G1':[],'AvgDegree_G1':[], '#Edges_G1':[]}
...
#             print(f'Iteration {i+1}')
        ...
        if R == 1:
            if not All_nodes[0]:
                ...
                while (flag == 0) & (count <= 20):
                    ...
                    if (first_node.nei[0] != second_node) & (first_node.nei[0].name != second_node.nei[0].name):
                        flag = 1
                        j += 1 
        ...
        if R == 3:
            if not All_nodes[0]:
                ...
                while (flag == 0) & (count <= 20):
                    ...
                    if (first_node.name != second_node1.nei[0].name) & (first_node.name != second_node2.nei[0].name):
                        if (first_node.nei[0].name != second_node1.nei[0].name) & (first_node.nei[0].name != second_node2.nei[0].name):
                            flag = 1
        ...
        if R == 4:
            if not All_nodes[2]:
                ...
                [deg3_node2, deg3_node3] = [m for m in All_nodes[2] if m.name == deg3_node1.name if m != deg3_node1]
                count = 0
                flag = 0
